...
### 5a.1 Benötigte Messwerte (Übersicht) ✅ (alle Zeilen umgesetzt)
...
| Windrichtung (°) | Regenschutz-Zusatzkriterium, optional (7) | ✅ konfigurierbare Fremd-State-ID (`windDirectionStateId`); haus20a nutzt `davis.0.sensors.tx1.windDirAvg10Min` |
...
- Eingang: Regen-Sensor-State-ID (extern verlinkt, `subscribeForeignStates`), optional Windrichtung (`IWeatherConfig.windDirectionStateId`).
- Regel: bei Regen (unbedingt) bzw. bei Regen + Windrichtung auf Fensterseite (± `IShutterConfig.rainProtectionWindDirectionToleranceDeg`, sofern pro Rolladen gesetzt — sonst wie zuvor unbedingt bei jedem Regen) → Rolladen auf konfigurierte Schutzposition fahren (z. B. nicht ganz zu, damit Fensterbank/Rahmen nicht nass wird, oder ganz zu je Fenstertyp — konfigurierbar). Default-Zielposition, sofern `rainTargetPercent` nicht explizit gesetzt ist: `protectedPosition(coveringType)` aus `covering-types.ts` (siehe 2a.5) — 100 % für `rolladen`/`raffstore`/`lamellen`, aber 0 % (einfahren) für `markise`, da eine ausgefahrene Markise selbst die im Regen gefährdete Stellung ist.
- Windrichtungs-Filter ist bewusst pro Rolladen opt-in (`rainProtectionWindDirectionToleranceDeg` undefiniert lassen = altes, unbedingtes Verhalten unverändert) und "fail open" Richtung Schutz: fehlt `orientation`, die Toleranz oder gerade der Windrichtungs-Messwert, wird trotzdem unbedingt geschützt, statt den Regenschutz stillschweigend zu deaktivieren.
...
- `rain-protection.ts`: analog Sonnenschutz-Modul, eigene Bewertungsfunktion; nutzt für die Windrichtungs-Toleranzprüfung dieselbe `isWithinOrientationWindow()` wie der Azimut-Ansatz in 6.2 (identische, wraparound-sichere ±°-Fenster-Logik).
...
`admin/index_m.html` + `admin/shutters.js` + `admin/words.js` sind die ausschließlich verwendete Oberfläche; dynamische Feld-Anzeige (z. B. je `driverType`/`coveringType` relevante State-ID-Felder), Buttons (Scan, Kalibrieren) und Instanz-/State-Auswahlfelder werden per eigenem JS (DOM-Sichtbarkeit, `sendTo()`-Aufrufe, `openStatePicker()`) statt über native JSONConfig-Feldtypen (`instance`, `hidden`-Ausdrücke, Custom-Buttons) umgesetzt.
...
- Regenschutz: Regen-Sensor-State-ID und Windrichtung-State-ID (optional) im globalen Panel `weather` (siehe 5a.1); pro Rolladen `rainProtectionEnabled`-Checkbox plus, sofern aktiviert und eine Ausrichtung gesetzt ist, das Feld "Regenschutz: Windrichtungs-Toleranz" (`rainProtectionWindDirectionToleranceDeg`) und die (bislang nicht im Admin-UI exponierte) Zielposition `rainTargetPercent`. Kein eigenes `rainProtection`-Panel.
...
## 11. Offene Fragen für Nutzer (vor Umsetzung klären) ✅ (keine offenen Fragen mehr — iCal-Integration wird nicht benötigt, Windrichtung beantwortet und umgesetzt, siehe 5a.1/7)
