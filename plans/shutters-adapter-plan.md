...
## 2. Fachliche Anforderungen → Module

| # | Anforderung | Modul (`src/lib/...`) |
|---|---|---|
| 1 | Einheitliche Alias-Steuerung aller Rolläden **über verschiedene Fremd-Adapter/Systeme hinweg** | `drivers/*.ts` + `shutter-controller.ts` |
| 2 | Behanghöhe ≠ lineare Laufzeit (Kalibrierungskurve) | `position-mapping.ts` |
| 3 | Tägliches Auf/Zu, abhängig von Wochentag/Wochenende/Feiertag/iCal, pro Zonen/Bereich | `scheduler.ts`, `holiday.ts`, `ical.ts` |
| 4 | Dämmerungsabhängige Steuerung (z. B. 30 min nach Ende der Dämmerung) | `twilight.ts` |
| 5 | Sonnenschutz nach Fensterausrichtung, Heizperioden-Ausnahme, Wolkenfilter | `sun-protection.ts` |
...
| 9 | Watchdog (hängender Antrieb erkennen) + Zustands-Recovery nach Adapter-Neustart | `watchdog.ts`, persistierte States |
| 10 | Sommer-Nachtauskühlung (abendliches Schließen zonenweise aussetzen/umkehren) | `night-cooling.ts` |
| 11 | Motorschutz (Mindestpause zwischen Fahrten) | `shutter-controller.ts` |
| 12 | Szenen/Vorgabepositionen | `scenes.ts` |
| 13 | Diagnose: Health-Check, Laufzeit-/Zyklen-Statistik, Simulationsmodus | `health-check.ts`, `runtime-stats.ts`, `drivers/simulation-driver.ts` |

## 2a. Treiber-Abstraktion für Fremdsysteme (Punkt 1 vertieft)
...
### 2a.2 Konkrete Driver-Implementierungen

| Driver | Datei | Verlinkte Fremd-States | Besonderheiten |
|---|---|---|---|
...
### 2a.4 Priorisierung der Driver-Implementierung (nach Verbreitung)
...
### 2a.5 Behangtyp-Unterscheidung (Rolladen vs. sonstiger Behang)
...
## 2b. Autoscan / Auto-Discovery der Rolläden
...
### 2b.2 Erkennungsstrategie je System

| System | Erkennungsmerkmal | Ableitung `driverType` |
|---|---|---|
...
│   │   ├── driverType          (string, ack=true, z.B. "homematic"|"hmip"|"knx"|"shelly"|"zigbee"|"zigbee2mqtt"|"tuya"|"somfy"|"velux"|"enocean"|"velbus"|"loxone"|"homey"|"mqtt"|"generic-relay"|"generic-position")
│   │   ├── coveringType        (string, ack=true, z.B. "rolladen"|"raffstore"|"markise"|"lamellen"|weitere, siehe 2a.5)
...
│   │   ├── sunProtectionOverrideUntil (string ISO / number ts, ack=true, gesetzt bei manueller Bedienung während aktivem Sonnenschutz; gültig bis lokal 24:00 desselben Tages)
│   │   ├── rainProtectionActive(boolean, ack=true)
│   │   ├── windProtectionActive(boolean, ack=true, aktuell tatsächlich wirksam)
│   │   ├── windProtectionEnabled(boolean, ack=false, Konfigurationsschalter — Default abhängig von coveringType, siehe 7a/2a.5)
│   │   ├── frostProtectionActive(boolean, ack=true, aktuell tatsächlich wirksam)
│   │   ├── frostProtectionEnabled(boolean, ack=false, Konfigurationsschalter — Default abhängig von coveringType, siehe 7b/2a.5)
│   │   ├── doorProtectionActive(boolean, ack=true)
│   │   ├── nightCoolingActive (boolean, ack=true, aktuell tatsächlich wirksam)
│   │   ├── nightCoolingEnabled(boolean, ack=false, Konfigurationsschalter, Default false)
...
## 4. Kalibrierung Behanghöhe → Laufzeit (Punkt 2)
...
## 5. Zeitsteuerung (Punkt 3 & 4)

- Basiskonfiguration pro Bereich (Zone): Öffnen-Zeit, Schließen-Zeit, je getrennt für Wochentag/Wochenende/Feiertag.
- Feiertagserkennung: npm-Paket für DE-Feiertage (bundesland-abhängig) oder eigene iCal-Quelle.
- iCal-Integration: pro Ereignis-Titel-Konvention (analog `resolvePlanFromIcalTitle` in irrigation) z. B. "Rolläden auf 07:00" im Kalender überschreibt Tagesdefault für diesen Tag.
...
## 5a. Zentrale Wetterdatenbeschaffung (mit Fallback auf externen Wetterdienst)
...
### 5a.1 Benötigte Messwerte (Übersicht)

| Messwert | Verwendet von | Nur mit eigenem Sensor sinnvoll? |
|---|---|---|
| Solarstrahlung (W/m²) | Sonnenschutz primär (6.1), Bewölkungsgrad-Eigenberechnung (6.2) | Nein — auch per Wetterdienst-API verfügbar |
| Windgeschwindigkeit + Böenspitze (km/h) | Windschutz (7a) | Nein — auch per Wetterdienst-API verfügbar |
| Windrichtung (°) | Regenschutz, optional (7) | Nein |
| Niederschlag (Regen ja/nein bzw. mm) | Regenschutz (7) | Nein |
| Außentemperatur (°C) | Frostschutz (7b), Hitzeschutz-Filter (6.5a), Heizperioden-Fallback (6.3) | Nein |
| Taupunkt bzw. Luftfeuchte (%) | Frostschutz-Kombikriterium (7b), Bewölkungsgrad-Eigenberechnung Modell B (6.2) | Nein |
| Luftdrucktrend (hPa/3h) | Bewölkungsgrad-Eigenberechnung Modell B, optionale Verfeinerung (6.2) | Nein, aber optional entbehrlich |
| Innentemperatur (°C, je Zone) | Nachtauskühlung (7c) | **Ja** — ein Wetterdienst kennt keine Innenraumtemperatur, hierfür ist zwingend ein eigener Sensor je Zone nötig; ohne eigenen Sensor ist Nachtauskühlung (7c) für die betroffene Zone deaktiviert |
| Vorhersage-Tagesmaximaltemperatur (°C) | Hitzeschutz-Vorabsenkung (6.5b) | Nein — im Gegenteil, dies ist praktisch **nur** per Wetterdienst-Vorhersage sinnvoll verfügbar, eine eigene Wetterstation liefert nur Live-Messwerte, keine Vorhersage |

### 5a.2 Priorität je Messwert: eigener Sensor vor Wetterdienst
...
### 5a.3 Externer Wetterdienst-Fallback
...
### 6.2 Optionale Erweiterung: Azimut-/Elevation-basierter Ansatz (M5b)
...
### 6.4 Manueller Override während aktivem Sonnenschutz ("Tagessperre")
...
## 7a. Windschutz / Sturmwarnung (Sicherheitsfunktion, höchste Priorität)
...
### 9a.1 Watchdog: hängender Antrieb
...
### 9a.2 Zustands-Recovery nach Adapter-Neustart
...
### 9a.3 Notify-Integration (optional)
...
## 10. Umsetzungsreihenfolge (Milestones)
...
9. **M6c — Watchdog & Zustands-Recovery**: `watchdog.ts`, persistierte Override-/Fahrzustände, Recovery-Logik beim `ready`-Handler (siehe 9a); optional Notify-Integration (9a.3).
...
13. **M8 — Tests & Release**: Unit-Tests für `position-mapping`, `sun-protection`, `rain-protection`, `wind-protection`, `frost-protection`, `night-cooling`, `scheduler`, **je Driver mit gemockten Fremd-States**; Integrationstest via `@iobroker/testing`; Lint; Adapter-Checker; README-Dashboard-Hinweis (9e); erster Release (`npm run release patch`).

## 10a. Endnutzer-Bedienkonzept (Einfachheit als Designziel)
...
### 10a.1 Reduzierte State-Oberfläche für Endnutzer
...
### 10a.2 Physische Taster/Fernbedienungen bleiben die primäre Bedienung
...
### 10a.3 Einfache Voreinstellungen statt Pflichtkonfiguration
...
### 10a.4 Zentrale Schnellaktionen statt Einzelsteuerung
...
### 10a.5 Sprachsteuerung/Smart-Home-Integration ohne Zusatzaufwand
...
### 10a.12 Inline-Hilfetexte in der Admin-UI
...
### 10a.13 Test-Button pro Rolladen
...
## 11. Offene Fragen für Nutzer (vor Umsetzung klären)
